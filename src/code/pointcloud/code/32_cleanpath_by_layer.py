...
# -*- coding: utf-8 -*-
"""
31_plane_detect_then_slice_by_step_or_layer.py (融合 B样条路径规划)

在原 31_ 的基础上融合 12_ 的 B样条拟合功能:
- 平面检测 + 分层切片(保持不变)
- 对每个非平面分层应用 B样条拟合生成清洁路径
- 支持层间连接器(smooth/retract/straight)
- 可视化路径 + 法向量 + 方向箭头
"""
...
SMOOTH_S            = 0.002     # B样条平滑系数
SAMPLES             = 300       # 每层拟合后初始采样点数
...
STITCH_MODE         = 'smooth'  # 'smooth', 'straight', 'retract'
...
def order_cycle_by_nearest_neighbor(pts2: np.ndarray) -> np.ndarray:
    ...
    从距离质心最远的点开始（通常是边界点），每次选择最近的未访问点。
    增加累积角度限制，确保只形成一个简单闭环（总转角≈360°）
    """
    n = len(pts2)
    if n < 3:
        return np.arange(n)
...
def order_cycle_by_alpha_shape(pts2: np.ndarray, alpha: float = 0.15) -> np.ndarray:
    ...
    cumulative_angle = 0.0
    prev_vec = None
    max_allowed_angle = 1.95 * np.pi  # 降低到351°，更严格避免打转（之前是396°）
...
def orient_normals_inward(points: np.ndarray, normals: np.ndarray, centroid: np.ndarray,
    ...
    """调整法向方向：朝向质心(inward=True)或远离质心，可强制z向上"""
...
def process_layer_bspline(layer_points: np.ndarray, 
    ...
    """
    对单层点云直接提取边界点生成路径（不做B样条拟合）
    如果检测到不连续区域，只使用最大的连续区域
    """
    ...
    try:
        ...
        order = order_cycle_by_alpha_shape(layer2, alpha=0.15)  # alpha稍小以保留更多细节
...
def main():
    ...
    if kd is not None and len(remain_layers) > 0:
        ...
        print(f"[INFO] Generating B-spline paths for {len(remain_layers)} remain layers (从最上层开始)...")
        for i, layer in enumerate(remain_layers):
            layer_pts = layer['points']
            path_pts, path_nrm = process_layer_bspline(
                layer_pts, pcd, kd,
                smooth_s=SMOOTH_S,
                samples=SAMPLES,
                step=PATH_STEP,
    ...
    # === 绘制路径（B样条拟合结果） ===
    if DRAW_PATHS and len(remain_polys_vis) > 0:
