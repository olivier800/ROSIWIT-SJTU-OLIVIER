...
import sensor_msgs.point_cloud2 as pc2   # ADD THIS LINE
...
class UrinalDetector:
    def __init__(self, service):
        ...
        self.predefined_rpy = service.predefined_rpy
        self.tool_pointing_height = service.tool_pointing_height
        self.roi_min = service.roi_min
        self.roi_max = service.roi_max

        # Load parameters from ROS parameter server
        self.load_parameters()        

        # PointCloudProcessor for cropping/downsampling (manual input)
        ...
        self.pc_processor.set_process_callback(self.process_pointcloud)

        self.segmenter = PointCloudSegmenter(target_class='toilet',
                                             color_img_topic=self.color_img_topic,
                                             pointcloud_topic=self.pointcloud_topic, 
                                             pc_processor_callback = self.pc_processor.pointcloud_callback)


        # Visualization publisher
        self.processed_pc_pub = rospy.Publisher('preprocessed_pointcloud', PointCloud2, queue_size=1)

        self.is_active = True
        rospy.loginfo("UrinalDetector initialized (segmenter integration)")
    ...
    def load_parameters(self):
        ...
        self.pointcloud_topic = rospy.get_param("~urinal_detector/pointcloud_topic", "/camera/depth/points")
        self.color_img_topic = rospy.get_param("~urinal_detector/color_img_topic", "/camera/color/image_raw")

        self.path_expand = rospy.get_param("~urinal_detector/path_expand")
        ...
        self.tool_pointing_x_offset_ratio = rospy.get_param("~urinal_detector/tool_pointing_x_offset_ratio", 0.12)
        
        # Path filtering parameters (for removing false closures in open shapes)
        self.enable_path_filter = rospy.get_param("~urinal_detector/enable_path_filter", True)
        self.path_filter_max_dist = rospy.get_param("~urinal_detector/path_filter_max_dist", 0.03)
        self.path_filter_min_segment = rospy.get_param("~urinal_detector/path_filter_min_segment", 5)
    ...
    def publish_processed_pointcloud(self, points):
        try:
            ...
            # Create point cloud with XYZ format
            processed_pc = pc2.create_cloud_xyz32(header, points)
            
            # Publish the processed point cloud
            rospy.loginfo(f"Published processed point cloud with {len(points)} points")
            self.processed_pc_pub.publish(processed_pc)
            
            rospy.loginfo_once("Published processed point cloud for sink detection")
            
        except Exception as e:
            rospy.logwarn(f"Failed to publish processed point cloud: {str(e)}")
    ...
    def process_pointcloud(self, points):
        ...
        self.publish_processed_pointcloud(points)
        try:
            if len(points) < 30:
                rospy.logwarn("UrinalDetector: Not enough points for detection")
                return
            clean_path = self.generate_clean_path(points)
            
            # Alpha Shape algorithm already includes orientation (Nx6)
            # Legacy spiral needs to add orientation
            if not self.use_alpha_shape and clean_path is not None:
                clean_path = self.add_direction(clean_path, self.tool_pointing_height)
            
            if clean_path is not None:
                self.service.process_detection_result(clean_path)
        except Exception as e:
            rospy.logerr(f"UrinalDetector error: {str(e)}")
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'pc_processor'):
            self.pc_processor.cleanup()
