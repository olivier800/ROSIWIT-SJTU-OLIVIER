...
def load_params():
    ...
    p["prompt"]         = rospy.get_param("~prompt", "请请识别可以用马桶刷清洁的马桶内腔部分，用bounding box框出，并以json格式返回。")
    # p["prompt"]         = rospy.get_param("~prompt", "请识别可以用马桶刷清洁的马桶内腔部分，注意不要包含马桶外壁，用bounding box框出，并以json格式返回。")
    # p["prompt"]         = rospy.get_param("~prompt", "请识别洗手池,不要包含洗手台的表面，用bounding box框出，并以json格式返回。")
    # p["prompt"]         = rospy.get_param("~prompt", "请识别洗手台表面，用bounding box框出，并以json格式返回。")
