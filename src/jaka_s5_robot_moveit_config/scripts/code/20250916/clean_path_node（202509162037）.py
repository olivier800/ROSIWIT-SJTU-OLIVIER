...
def load_params():
    ...
    p["trim_top"]            = rospy.get_param("~trim_top", 0.00)              # 顶部裁剪高度（米），去除顶部噪声
    ...
    p["uniform_mode"]        = rospy.get_param("~uniform_mode", "mesh_poisson")         # 均匀化算法选择：fps/mesh_poisson/adaptive_blue_noise
...
def _reconstruct_mesh_from_pcd(
    ...
    if densities.size > 0 and trim_pct > 0.0:
        ...
        rospy.loginfo("[clean_path_node] mesh recon: poisson trim %.1f%% -> V %d → %d", trim_pct, before_v, after_v)
    else:
        rospy.loginfo("[clean_path_node] mesh recon: poisson trim disabled (trim=%.1f%%)", trim_pct)

    mesh.compute_triangle_normals()
    mesh.compute_vertex_normals()
    rospy.loginfo("[clean_path_node] mesh recon: done (V=%d, F=%d)", len(mesh.vertices), len(mesh.triangles))
    return mesh
...
def _snap_points_to_cloud(xyz, ref_pcd, max_dist=0.01, keep_z=True):
    ...
    rospy.loginfo("[clean_path_node] post-snap: kept %d, dropped %d (max_dist=%.3f, keep_z=%s)",
...
def uniformize_pcd(
    ...
    if uniform_mode == "adaptive_blue_noise":
        ...
        alpha   = float(rospy.get_param("~abn_alpha", 1.1))           # 冲突半径系数：控制点间最小距离，越大越稀疏
        kn      = int(rospy.get_param("~abn_knn_k", 8))               # k近邻数量：用于估算局部点密度的邻居点数
        s_min   = float(rospy.get_param("~abn_s_min", 0.003))         # 最小间距：允许的最小点间距离（米）
        s_max   = float(rospy.get_param("~abn_s_max", 0.010))         # 最大间距：允许的最大点间距离（米）
        s_fixed = float(rospy.get_param("~abn_fixed_spacing", 0.0))   # 固定间距：>0时使用固定值，否则自适应估算
        tgtN    = int(rospy.get_param("~abn_target_points", 0))       # 目标点数：>0时逼近此点数，通过调整间距实现
        ...
        rospy.loginfo("[clean_path_node][adaptive_blue_noise]: 输入=%d点 -> 输出=%d点 (全局间距≈%.1f毫米, 冲突半径系数α=%.2f, k近邻=%d, 间距范围=%.1f-%.1f毫米%s)",
                      base_pts.shape[0], len(sel), s*1000.0, alpha, kn, s_min*1000.0, s_max*1000.0,
                      f", 目标点数={tgtN}" if tgtN>0 else "")
        return out
    ...
    def publish_all(self):
        ...
        if self.cached_processed_xyz is not None:
            ...
            if not self._logged_processed:  
                rospy.loginfo("[clean_path_node] publish: /%s (%d pts)", self.p["processed_pointcloud_topic"], len(self.cached_processed_xyz))
