...
FIXED_BBOX = [544, 0, 836, 416]  # ←←← 按你的画面分辨率填
...
def load_params():
    ...
    # SAM (use hardcoded values from config section)
    p["sam_checkpoint"] = rospy.get_param("~sam_checkpoint", SAM_CHECKPOINT)
    p["sam_model_type"] = rospy.get_param("~sam_model_type", SAM_MODEL_TYPE)
    p["device"]         = rospy.get_param("~device", SAM_DEVICE)
    ...
    # Temporal fusion
    p["num_trials"] = rospy.get_param("~num_trials", 2)
    ...
    p["size_profile"] = prof
    if prof == "toilet":
        p["a_min"], p["a_max"], p["b_min"], p["b_max"] = 0.25, 0.45, 0.35, 0.65
        p["depth_med_min_mm"], p["depth_med_max_mm"] = 80.0, 180.0
    else:
        p["a_min"], p["a_max"], p["b_min"], p["b_max"] = 0.35, 0.65, 0.25, 0.45
        p["depth_med_min_mm"], p["depth_med_max_mm"] = 120.0, 220.0
