...
FIXED_BBOX = [524, 0, 856, 416]  # ←←← 按你的画面分辨率填
...
def load_params():
    ...
    # ==================== 时域融合配置 ====================
    p["num_trials"] = rospy.get_param("~num_trials", 2)  # 执行试验次数，用于多次采样选最优结果
    ...
    # ==================== 图像增强配置 ====================
    p["enhance_mode"] = rospy.get_param("~enhance_mode", "darkboost")  # 图像增强模式，用于改善SAM分割效果
...
# ====================== 点云预处理：trim -> voxel -> ror -> mls (尝试) ======================
def preprocess_pcd(pcd: o3d.geometry.PointCloud, params: dict) -> o3d.geometry.PointCloud:
    """
    统一的点云预处理流程，保证顺序为：
      1) trim (去除无效点、z<=0等)
      2) voxel 下采样
      3) radius outlier removal (ROR)
      4) 平滑 (尝试使用 Open3D 可用的平滑函数，如 filter_smooth_simple)

    返回处理后的点云（如果输入为空则直接返回）
    """
    if pcd is None or len(pcd.points) == 0:
        return pcd

    # 1) Trim: 去除非有限值与 z<=0 的点
    pts = np.asarray(pcd.points)
    finite_mask = np.isfinite(pts).all(axis=1) & (pts[:, 2] > 0)
    if finite_mask.sum() == 0:
        return o3d.geometry.PointCloud()
    if finite_mask.sum() != pts.shape[0]:
        idxs = np.nonzero(finite_mask)[0].tolist()
        pcd = pcd.select_by_index(idxs)

    # 2) Voxel 下采样：融合模式为 point_union 时优先使用 union_voxel_m，否则使用 voxel_size
    voxel_size = params.get("union_voxel_m") if params.get("fusion_mode") == "point_union" else params.get("voxel_size", 0.005)
    try:
    ...
    # 3) Radius Outlier Removal (ROR)
    # 使用一个保守的半径与最小邻居数，若失败则跳过
    try:
        nb_points = int(max(1, params.get("outlier_nb", 20)))
        radius = float(params.get("ror_radius_m", 0.02))  # 默认为 2cm
        pcd, ind = pcd.remove_radius_outlier(nb_points=nb_points, radius=radius)
    except Exception:
        # 如果 remove_radius_outlier 不可用或出错，尝试统计法去噪
        try:
            nb = int(max(1, params.get("outlier_nb", 20)))
            std = float(params.get("outlier_std", 2.0))
            pcd, ind = pcd.remove_statistical_outlier(nb_neighbors=nb, std_ratio=std)
        except Exception:
            pass

    # 4) 平滑 (MLS/简单平滑)：尽量使用Open3D提供的简单平滑接口
    try:
        if hasattr(pcd, 'filter_smooth_simple'):
            # 一次迭代的简单平滑（若Open3D支持）
            pcd = pcd.filter_smooth_simple(number_of_iterations=1)
        elif hasattr(pcd, 'filter_smooth_taubin'):
            pcd = pcd.filter_smooth_taubin(number_of_iterations=1)
    except Exception:
        # 若都不可用则跳过
        pass
...
class MaskedNode:
    ...
    def cb(self, rgb_msg, depth_msg, cam_info):
        ...
        if self.temporal.ready():
            ...
            if self.params["fusion_mode"] == "point_union":
                ...
                # 统一调用预处理流程：trim -> voxel -> ror -> mls
                pcd_cam = preprocess_pcd(pcd_cam, self.params)
            else:  # depth_robust
    ...
    def finish_trial_and_score(self, pcd_cam, mask_fused, depth_fused_u16, intr, rgb, depth_raw_u16):
        t = self.trial_idx + 1
        # 先对点云做统一预处理，保证顺序为 trim->voxel->ror->mls
        pcd_cam = preprocess_pcd(pcd_cam, self.params)
