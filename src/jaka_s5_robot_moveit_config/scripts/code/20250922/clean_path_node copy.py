...
def load_params():
    ...
    # sink -> 0.01, toilet -> 0.07
    p["trim_bottom"]         = rospy.get_param("~trim_bottom", 0.0)           # 底部裁剪高度（米），去除底部噪声
    ...
    # 覆盖式路径规划参数（螺旋）
    # path_mode: spiral
    p["path_mode"]          = rospy.get_param("~path_mode", "spiral")                 # 路径模式: 螺旋
...
class CleanPathNodeOnceRepublish(object):
    ...
    def try_process_once(self, _evt):
        ...
        try:
            ...
            # ★ 表面贴合回吸，避免悬空
            ref_for_snap = pcd_clean if self.p.get("path_snap_ref", "processed") == "processed" else pcd_uni
            dense_xyz = _snap_polyline_to_surface(
                dense_xyz,
                ref_for_snap,
                mode=self.p.get("path_snap_mode", "plane"),
                radius=self.p.get("path_snap_radius", 0.02),
                knn=self.p.get("path_snap_knn", 30),
                max_dev=self.p.get("path_max_dev", 0.01),
                keep_z=self.p.get("path_keep_z", False)
            )
            rospy.loginfo("[clean_path_node] 路径(%s): 关键点=%d, 稠密(贴合)=%d", str(self.p.get("path_mode")), len(key_xyz), len(dense_xyz))
