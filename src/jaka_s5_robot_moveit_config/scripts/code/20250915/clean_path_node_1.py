...
def load_params():
    ...
    # 预处理
    p["voxel"]               = rospy.get_param("~voxel", 0.000)
    ...
    p["uniform_mode"]        = rospy.get_param("~uniform_mode", "fps")
    p["target_points"]       = rospy.get_param("~target_points", 60000)
    p["use_mls"]             = rospy.get_param("~use_mls", True)
    # 方法1: FPS
    p["radius_rel"]          = rospy.get_param("~radius_rel", 1) # 相对均距的采样半径,可以控制点云密度,越小越多
    ...
    # 路径参数（螺旋关键点推进）
    p["slice_step"]          = rospy.get_param("~slice_step", 0.005)
...
def preprocess_pcd(
    ...
    rospy.loginfo("[clean_path_node] 预处理开始 (voxel=%.3f, ROR=%.3f/%d, trim_top=%.3f, trim_bottom=%.3f)",
                  voxel, ror_radius, ror_min_pts, trim_top, trim_bottom)

    pcd = trim_by_height(pcd, trim_bottom=trim_bottom, trim_top=trim_top)
    if voxel and voxel > 0:
        pcd = pcd.voxel_down_sample(voxel_size=voxel)
    before_ror = len(pcd.points)
    ...
    rospy.loginfo(
        "[clean_path_node] 预处理: 完成 (voxel=%.3f, ROR=%.3f/%d) pts: %d -> %d",
        voxel, ror_radius, ror_min_pts, before_ror, after_ror
    )
    return pcd
