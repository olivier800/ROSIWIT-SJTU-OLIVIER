...
bool loadParameters(ros::NodeHandle& nh, ControllerParams& params) {
    ...
    // 数据记录参数
    nh.param("enable_logging", params.enable_logging, true);
    nh.param("log_file_path", params.log_file_path, string("/home/olivier/wwx/code/"));
...
// 生成带时间戳的日志文件名
string generateLogFileName(const string& log_dir) {
    // 创建目录(如果不存在)
...
// 初始化日志文件
std::ofstream initLogFile(const string& filename) {
    ...
    // 写入CSV表头
    log_file << "timestamp,index,";
    log_file << "pos_x,pos_y,pos_z,";
    log_file << "normal_x,normal_y,normal_z,";
    log_file << "desired_fx,desired_fy,desired_fz,";
    log_file << "actual_fx,actual_fy,actual_fz,";
    log_file << "error_fx,error_fy,error_fz,";
    log_file << "adjustment_x,adjustment_y,adjustment_z,";
    log_file << "torque_tx,torque_ty,torque_tz" << std::endl;
    
    ROS_INFO("Force data logging to: %s", filename.c_str());
    return log_file;
}
...
int main(int argc, char *argv[]) {
    ...
    // 初始化日志文件
    std::ofstream force_log_file;
    auto start_time = std::chrono::high_resolution_clock::now();
    
    if (params.enable_logging) {
        string log_filename = generateLogFileName(params.log_file_path);
        force_log_file = initLogFile(log_filename);
        if (!force_log_file.is_open()) {
            ROS_WARN("Logging disabled due to file open error");
    ...
    JointValue home_joint = {-2.047, 1.596, -0.601, 0.571, 1.552, -4.403}; //末端转180
    // toilet
    ...
    while(index < arrow_interpolated_.size()) {  
        ...
        // 记录数据到日志文件
        if (params.enable_logging && force_log_file.is_open()) {
            auto current_time = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                current_time - start_time);
            
            force_log_file << std::fixed << std::setprecision(3)
                          << duration.count() << ","
                          << index << ","
                          << target_tip_position[0] << "," 
                          << target_tip_position[1] << "," 
                          << target_tip_position[2] << ","
    ...
    // 关闭日志文件
    if (params.enable_logging && force_log_file.is_open()) {
        force_log_file.close();
        ROS_INFO("Force data log saved successfully");
    }
