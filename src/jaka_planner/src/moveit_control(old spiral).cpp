...
public:
    NormalArrowSubscriber(ros::NodeHandle& nh) : nh_(nh) {
        sub_ = nh_.subscribe("/spiral_path_marker_array", 1, &NormalArrowSubscriber::markerArrayCallback, this);
    }
...
private:
    void markerArrayCallback(const visualization_msgs::MarkerArray::ConstPtr& msg) {
        for (const auto& marker : msg->markers) {
            ...
            ifGetArrow = true;
            cout<<"Received "<<arrow_infos_.size()<<" arrows from /clean_path_key_normals (only first message stored)"<<endl;
...
public:
    ...
    CenterPointSubscriber(ros::NodeHandle& nh) : nh_(nh) {
        sub_ = nh_.subscribe("/spiral_center_point", 1, &CenterPointSubscriber::centerCallback, this);
    }


    void centerCallback(const geometry_msgs::PointStamped::ConstPtr& msg)
    {
        center_point = 1000*Eigen::Vector3d(msg->point.x,msg->point.y,msg->point.z+0.62);    
...
bool loadParameters(ros::NodeHandle& nh, ControllerParams& params) {
    // 力控参数
    nh.param("desired_force", params.desired_force, 2.0);
    nh.param("kp", params.kp, 0.01);
    nh.param("kd", params.kd, 0.06);
    nh.param("mass", params.mass, 0.003);
    nh.param("kpZ", params.kpZ, 0.5);
    nh.param("kdZ", params.kdZ, 0.1);
    
    // 运动参数
    nh.param("interpolation_dist", params.interpolation_dist, 0.6);
    nh.param("servo_stepnum", params.servo_stepnum, 1.0);
    nh.param("joint_move_speed", params.joint_move_speed, 50.0);
...
int main(int argc, char *argv[]) {
    ...
    // 移动到初始位置
    CartesianPose home_pose = {59.727, -371.07, 517.476, -3.017, -0.176, -2.484};
    ...
    home_joint = {-5.07, 1.856, -0.795, 0.628, 1.524, -1.054};
    ...
    // 开始力控运动
    int index = 0;
    while(index < arrow_interpolated_.size()*3/4 ) {  
        TorqSensorData forcedata;
        Eigen::Vector3d normal = arrow_interpolated_[index].normal; 
        Eigen::Vector3d target_position = arrow_interpolated_[index].position; // 机械臂目标路径点 
        controllerX.setDesiredForce(params.desired_force*normal[0]);
        controllerY.setDesiredForce(params.desired_force*normal[1]);
        controllerZ.setDesiredForce(params.desired_force*normal[2]);
        ...
        //adjustment[2] = 0.0;
        //ROS_INFO("force: [%.2f, %.2f, %.2f] N", 
        //                forceX,forceY,forceZ);
        //ROS_INFO("adjust: [%.2f, %.2f, %.2f] N", 
        //                adjustment[0],adjustment[1],adjustment[2]);
        target_position += adjustment;
        CartesianPose target_cart;
        target_cart.tran = {target_position[0], target_position[1], target_position[2]};
