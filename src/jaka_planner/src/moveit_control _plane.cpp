...
private:
    void markerArrayCallback(const visualization_msgs::MarkerArray::ConstPtr& msg) {
        for (const auto& marker : msg->markers) {
            ...
            ifGetArrow = true;
            cout<<"Received "<<arrow_infos_.size()<<" arrows from /clean_path_plane_normals (only first message stored)"<<endl;
...
bool loadParameters(ros::NodeHandle& nh, ControllerParams& params) {
    // 力控参数 - XY位置精度强化，Z轴保持柔顺
    nh.param("desired_force", params.desired_force, 5.0);  // 适中的期望力即可
    nh.param("kp", params.kp, 0.13);   // 进一步增强XY位置约束，严格跟随路径
    nh.param("kd", params.kd, 0.18);   // 进一步增大XY阻尼，快速收敛到路径
    nh.param("mass", params.mass, 0.0015);  // 进一步减小质量，XY快速响应
    nh.param("kpZ", params.kpZ, 0.04);  // Z轴保持柔顺，适应表面
    nh.param("kdZ", params.kdZ, 0.10);  // Z轴适度阻尼
...
int main(int argc, char *argv[]) {
    setlocale(LC_ALL, "");
    ros::init(argc, argv, "moveit_control_sink");
    ...
    JointValue home_joint = {-1.930, 1.520, -0.525, 0.570, 1.554, -1.134};
