...
public:
    NormalArrowSubscriber(ros::NodeHandle& nh) : nh_(nh) {
        sub_ = nh_.subscribe("/clean_path_remain_normals", 1, &NormalArrowSubscriber::markerArrayCallback, this);
    }
...
private:
    void markerArrayCallback(const visualization_msgs::MarkerArray::ConstPtr& msg) {
        for (const auto& marker : msg->markers) {
            ...
            ifGetArrow = true;
            cout<<"Received "<<arrow_infos_.size()<<" arrows from /clean_path_remain_normals (only first message stored)"<<endl;
...
int main(int argc, char *argv[]) {
    ...
    JointValue home_joint = {1.233, 1.484, -1.051, 1.215, 1.577, -0.867};
